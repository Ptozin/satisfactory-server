...
if [[ "${SKIPUPDATE,,}" != "true" ]]; then
    ...
    # steamcmd +force_install_dir /config/gamefiles +login anonymous +app_update 1690800 -beta public -validate -manifest 30903977 +quit
    steamcmd +force_install_dir /config/gamefiles +login anonymous +app_update "$STEAMAPPID" -beta "$STEAMBETAFLAG" -manifest 19824343 validate +quit
